...
export async function upsertUser(githubId, username, avatarUrl) {
  await initDb()
...
export async function getRepoReputation(owner, name) {
  await initDb()
  const result = await pool.query(
    `SELECT
...
export async function getUserRepForRepo(userId, owner, name) {
  await initDb()
  const result = await pool.query(
...
export async function addOrUpdateReputation(owner, name, userId, type) {
  await initDb()
  await pool.query(
...
export async function deleteReputation(userId, owner, name) {
  await initDb()
  await pool.query(
...
export async function getRepoComments(owner, name, limit = 100) {
  await initDb()
  const result = await pool.query(
...
export async function addComment(owner, name, userId, content) {
  await initDb()
  const result = await pool.query(
...
export async function deleteCommentAdmin(commentId, adminId, reason) {
  await initDb()
...
export async function getAllComments(page = 1, search) {
  await initDb()
  const limit = 20
  const offset = (page - 1) * limit
...
export async function getModLog(page = 1) {
  await initDb()
  const limit = 20
  const offset = (page - 1) * limit
...
export async function removeBlockedWord(word) {
  await initDb()
