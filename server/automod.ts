...
function getEnvBlockedWords(): string[] {
...
async function getBlockedWords(): Promise<string[]> {
  ...
  const dbWords = dbResult.rows.map((r: any) => r.word as string)
...
export async function checkComment(userId: number, content: string): Promise<AutomodResult> {
  // Word filter
